...
public abstract class GroupedPanelBase<TItem, TModel> : SidePanel where TItem : BaseViewModel, IGroupableItem, new() where TModel : class
{
    ...
    protected GroupedPanelBase(ControllerViewModel controller, IDispatcherService dispatcher) : base(controller, dispatcher) { }
    ...
    protected void AddItemToGroup(TItem item)
    {
        ...
        if (group == null)
        {
            group = new Group<TItem>(item.GroupKey, item.GroupSortOrder, Dispatcher);
