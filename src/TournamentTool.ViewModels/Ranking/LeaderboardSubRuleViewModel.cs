...
public class LeaderboardSubRuleViewModel : BaseViewModel
{
    ...
    public LuaLeaderboardScriptViewModel? SelectedScript
    {
        ...
        set
        {
            ...
            {
                var result = _dialogService.Show("Changing the script will reset all the custom variable values to default.\n" +
                                                 "Are you sure you want to change script?", "Changing script with modified values in custom variables", 
    ...
    public LeaderboardSubRuleViewModel(LeaderboardSubRule model, LeaderboardRuleViewModel rule, INotifyPresetModification notifyPresetModification, IDialogService dialogService, IDispatcherService dispatcher) : base(dispatcher)
    {
    ...
    {
        var result = _dialogService.Show("Are you sure you want to reset all custom variables in that sub rule to default?", 
            "Resetting custom variables to default", MessageBoxButton.YesNo, MessageBoxImage.Warning);
    ...
    {
        CustomVariables = new ObservableCollection<LuaCustomVariableViewModel>(Model.CustomVariables.Values.Select(v => new LuaCustomVariableViewModel(v, _notifyPresetModification, Dispatcher)));
